...
public class Expression {
    ...
    private BigDecimal evaluateExpression(BigDecimal op_1, BigDecimal op_2, String operand) { //Evaluates basic arithmetic
        switch (operand) {
            ...
            case "-":
                return op_1.subtract(op_2);
            case "^":
                return BigDecimal.valueOf(Math.pow(op_1.doubleValue(), op_2.doubleValue()));
            case "*":
                return op_1.multiply(op_2);
            case "/":
                return op_1.divide(op_2, MathContext.DECIMAL64); //I'm not sure why these are reversed, I must have done something wrong in the evaluation
            default:
